...
def export_res_LMM_reg_OC_ALLCOND():
    ...
    for band in band_list_short:
        ...
        df_LMM_paper = df_LMM_OC_short.query(f"band == '{band}'")
        df_LMM_paper['term'] = df_LMM_paper['term'].replace({'rf_metric_val' : 'regression', 'condoc_chl' : 'Ch-effect', 'rf_metric_val:condoc_chl' : 'regression:Ch interaction'})
        df_LMM_paper['ROI'] = [f"{_ROI_chunk[0]} {_ROI_chunk[1]}" for _ROI_chunk in [_ROI.split(' ') for _ROI in df_LMM_paper['ROI'].values]]

        estimate_list = ['regression', 'Ch-effect', 'regression:Ch interaction']
        ...
        for col_i, rf_metric in enumerate(paper_list_rf_metrics, start=1):
            ...
            for row_i, estimate in enumerate(estimate_list, start=1):
                ...
                df_sub = df_LMM_paper.query(f"rf_metric == '{rf_metric}' and term == '{estimate}'")

                vmin, vmax = df_sub["estimate"].min()-margin_factor, df_sub["estimate"].max()+margin_factor
                
                for j, phase_cycle in enumerate(phase_cycle_list):

                    df_c = df_sub.query(f"phase_cycle == '{phase_cycle}'")
                    ...
                    fig_paper.update_xaxes(
                        categoryorder="array",
                        categoryarray=ROI_order,
                        showticklabels=(row_i == len(estimate_list)),
                        row=row_i,
                        col=col_i
                    )
        ...
        filepath = os.path.join(path_paper_figure_export, f"fig05b_ALLCOND_REG_RF_{band}_{phase_protocol}.svg")
